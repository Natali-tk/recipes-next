...
export async function createRecipe(formData: FormData) {
  try {
    ...
    if (!name || ingredients.length === 0) {
      return {
        success: false,
        error: "Ім'я і хотя б один інградієнт обов'язкові"
      };
    }
    ...
    return { success: false, error: "Помилка при створенні рецепта!" };
  }
}
...
export async function updateRecipe(id: string, formData: FormData) {
  try {
    ...
    return { success: false, error: "Помилка при оновленні рецепта!" };
  }
}
...
export async function deleteRecipe(id: string) {
  try {
    ...
    return { success: false, error: "Помилка при видаленні рецепта!" };
  }
}